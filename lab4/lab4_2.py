...
for x in [3,5,7]:
    thresh3x3 = cv2.erode(thresh, kernel3x3, iterations=x)
    thresh5x5 = cv2.erode(thresh, kernel5x5, iterations=x)
    thresh10x10 = cv2.erode(thresh, kernel10x10, iterations=x)
    combine = np.hstack((thresh, thresh3x3, thresh5x5, thresh10x10))

    thresh_rect_3x5 = cv2.erode(thresh, kernel_rect_3x5, iterations=x)
    thresh_rect_5x3 = cv2.erode(thresh, kernel_rect_5x3, iterations=x)
    thresh_rect_10x3 = cv2.erode(thresh, kernel_rect_10x3, iterations=x)
    combine1 = np.hstack((thresh, thresh_rect_3x5, thresh_rect_5x3, thresh_rect_10x3))

    thresh_cross = cv2.erode(thresh, kernel_cross, iterations=x)
    thresh_ellip = cv2.erode(thresh, kernel_ellip, iterations=x)
    thresh_circle = cv2.erode(thresh, kernel_circle, iterations=x)
    combine2 = np.hstack((thresh, thresh_cross, thresh_ellip, thresh_circle))
